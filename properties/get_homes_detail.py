...
def batch(subset):
    ...
    logging.info(
        f"Queued {houses_df.shape[0]} records of houses information, uploading to "
        f"database.")
    upsert_dataframe(
        engine,
        houses_df,
...
n_threads = int(os.environ['N_THREADS_HOMES'])
Parallel(n_jobs=n_threads, prefer="threads")(
    delayed(batch)(properties.iloc[i:i + 500, :])
    for i in range(0, properties.shape[0], 500)
)
