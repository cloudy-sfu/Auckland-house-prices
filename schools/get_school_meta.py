...
chrome = ChromiumPage()
chrome.get("https://catalogue.data.govt.nz/api/3/action/datastore_search_sql?sql="
           "SELECT%20*%20from%20%224b292323-9fcc-41f8-814b-3c7b19cf14b3%22")
cf_bypass = CloudflareBypass(chrome)
cf_bypass.bypass()
response_json = chrome.json
chrome.quit()
...
schools['is_public'] = schools['Authority'].apply(
    lambda a: pd.NA if pd.isna(a) else not a.startswith("Private")
)
...
schools['open_date'] = pd.to_datetime(schools['DateSchoolOpened'], errors='coerce').dt.date
schools.fillna({"open_date": pd.NA}, inplace=True)
...
for col in [
    ...
    schools[col] = pd.to_numeric(schools[col], errors='coerce')
schools = schools.convert_dtypes()
